...
def fetch_books_report_data(org_id=None):
    query = Book.query
...
@role_required('admin', 'librarian')
def export_report():
    authorized, org_id = check_export_authorization()
    if not authorized:
    ...
    if report_type == 'books':
        if format_type == 'csv':
            return export_books_csv(org_id)
        elif format_type == 'xlsx':
            return export_books_excel(org_id)
        elif format_type == 'pdf':
            return export_books_pdf(org_id)
        else:
    ...
    try:
        if format_type == 'xlsx':
            ...
            class GenericNumberedCanvas(canvas.Canvas):
                ...
                def draw_page_number(self, page_count):
                    ...
                    self.drawString(54, 750, f"NOVA Library - {report_type.capitalize()} Report")
            ...
            title_style = ParagraphStyle(
                'TitleStyle', parent=styles['Heading2'], fontSize=14, leading=18, textColor=colors.HexColor('#1e3a8a')
            )
            subtitle_style = ParagraphStyle(
                'SubtitleStyle', parent=styles['Normal'], fontSize=8, leading=10, textColor=colors.HexColor('#64748b')
            )
            cell_style = ParagraphStyle(
                'CellStyle', parent=styles['Normal'], fontSize=7, leading=9
            )
            header_cell_style = ParagraphStyle(
                'HeaderCellStyle', parent=styles['Normal'], fontSize=7, leading=9, textColor=colors.white, fontName='Helvetica-Bold'
            )
            ...
            title_p = Paragraph(f"<b>NOVA INSTITUTE OF TECHNOLOGY</b>", title_style)
            subtitle_p = Paragraph(f"Nova Smart Library - {report_type.capitalize()} Report (Compiled: {datetime.utcnow().strftime('%Y-%m-%d')})", subtitle_style)
            ...
            table_data = [[Paragraph(f"<b>{col_name}</b>", header_cell_style) for col_name in cols]]
            
            for r in rows:
                row_cells = []
                for idx in range(len(cols)):
                    val = r.get(f"col{idx+1}", "")
                    row_cells.append(Paragraph(str(val), cell_style))
                table_data.append(row_cells)
                
            col_width = 504 / len(cols)
            main_table = Table(table_data, colWidths=[col_width]*len(cols))
            ...
            doc.build(elements, canvasmaker=GenericNumberedCanvas)
            buffer.seek(0)
            ...
            for r in rows:
                writer.writerow([r.get(f'col{i+1}', '') for i in range(len(cols))])
...
def export_books_print_endpoint():
    ...
    if not auth:
        return jsonify({"msg": "Access denied. Unauthorized."}), 403
    return export_books_print(org_id)


def export_books_csv(org_id=None):
    try:
...
def export_books_print(org_id=None):
    try:
        data = fetch_books_report_data(org_id)
