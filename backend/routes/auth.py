...
def get_public_organizations():
    orgs = Organization.query.filter_by(status='active').order_by(Organization.name.asc()).all()
...
def login():
    ...
    requested_org_id = data.get('org_id')
    ...
    if user.role != 'superadmin':
        if not requested_org_id:
            return jsonify({"msg": "Organization selection is required"}), 400
        if int(user.org_id) != int(requested_org_id):
            return jsonify({"msg": "This account does not belong to the selected organization."}), 403
