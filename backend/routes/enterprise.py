...
def get_reviews():
    ...
    if book_id:
        ...
        reviews = BookReview.query.order_by(BookReview.created_at.desc()).limit(20).all()
...
@jwt_required()
def get_seats():
    active_bookings = SeatBooking.query.filter_by(status='booked').all()
...
def book_seat():
    ...
    existing = SeatBooking.query.filter_by(seat_number=seat_number, status='booked').first()
    if existing:
        return jsonify({
            "success": False,
