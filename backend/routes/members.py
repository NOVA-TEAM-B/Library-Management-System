...
def get_members():
    ...
    # Calculate stats
    total_m = User.query.filter_by(role='member').count()
    active_m = User.query.filter_by(role='member', status='active').count()
    inactive_m = User.query.filter_by(role='member', status='inactive').count()
    
    last_month = datetime.utcnow() - timedelta(days=30)
    new_reg = User.query.filter(User.role == 'member', User.created_at >= last_month).count()
