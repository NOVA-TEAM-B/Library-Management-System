...
def issue_book():
    ...
    if role == 'member':
        if member_id != current_user_id:
            return jsonify({"msg": "Unauthorized to issue books for other members"}), 403
    elif role not in ['admin', 'librarian']:
    ...
    existing_issue = Issue.query.filter_by(book_id=book.id, member_id=member.id).filter(Issue.status.in_(['issued', 'overdue'])).first()
    if existing_issue:
        return jsonify({"msg": f"You already have an active checkout of '{book.title}'"}), 400
    ...
    if org_id is not None:
        if book.org_id != org_id or member.org_id != org_id:
    ...
    notif = Notification(
        user_id=member.id,
        title="Book Issued",
        message=f"Book '{book.title}' has been successfully issued. Due date: {issue.due_date.strftime('%Y-%m-%d')}.",
        org_id=org_id
    )
    db.session.add(notif)
    db.session.commit()
    
    # Broadcast alert via socketio if accessible
    from flask import current_app
    if hasattr(current_app, 'socketio') and current_app.socketio:
        try:
            current_app.socketio.emit('sync', {'event': 'book_issued', 'user_id': member.id}, broadcast=True)
            current_app.socketio.emit('notification', {
                'title': 'Book Issued Successfully',
                'message': f"The book '{book.title}' has been checked out to you.",
...
def return_book(issue_id):
    ...
    if role not in ['admin', 'librarian'] and issue.member_id != current_user_id:
        return jsonify({"msg": "Unauthorized to return this checkout"}), 403
    ...
    if hasattr(current_app, 'socketio') and current_app.socketio:
        try:
            current_app.socketio.emit('sync', {'event': 'book_returned', 'user_id': issue.member_id}, broadcast=True)
            current_app.socketio.emit('notification', {
                'title': 'Book Returned Successfully',
                'message': f"The book '{book.title if book else ''}' has been returned.",
                'timestamp': datetime.utcnow().strftime('%H:%M:%S'),
                'is_read': False
            }, broadcast=True)
        except Exception as e:
...
def get_fines():
    ...
    if claims.get('role') in ['admin', 'librarian']:
        ...
        query = Fine.query.filter_by(member_id=user_id)
        if org_id is not None:
            query = query.filter_by(org_id=org_id)
        fines = query.order_by(Fine.created_at.desc()).all()
        
    # Calculate total billing metrics
    total_logged_q = db.session.query(db.func.sum(Fine.amount))
    collected_q = db.session.query(db.func.sum(Fine.amount)).filter_by(status='paid')
    pending_q = db.session.query(db.func.sum(Fine.amount)).filter_by(status='pending')
    overdue_loans_q = Issue.query.filter_by(status='overdue')
    
    if org_id is not None:
        total_logged_q = total_logged_q.filter_by(org_id=org_id)
        collected_q = collected_q.filter_by(org_id=org_id)
        pending_q = pending_q.filter_by(org_id=org_id)
        overdue_loans_q = overdue_loans_q.filter_by(org_id=org_id)
        
    total_logged = total_logged_q.scalar() or 0.0
    collected = collected_q.scalar() or 0.0
    pending = pending_q.scalar() or 0.0
    overdue_loans = overdue_loans_q.count()
...
def pay_fine(fine_id):
    ...
    org_id = get_current_org_id()
    if org_id is not None and fine.org_id != org_id:
        return jsonify({"msg": "Unauthorized. Resource organization mismatch."}), 403
...
@issue_return_bp.route('/progress', methods=['POST'])
@jwt_required()
def save_reading_progress():
    data = request.get_json()
    if not data or not data.get('book_id') or 'current_page' not in data or 'total_pages' not in data:
        return jsonify({"msg": "Missing book ID, current page, or total pages"}), 400
    ...
    # Find active checkout first
    issue = Issue.query.filter_by(book_id=book_id, member_id=user_id).filter(Issue.status.in_(['issued', 'overdue'])).first()
    if not issue:
        # Fall back to any recent checkouts
        issue = Issue.query.filter_by(book_id=book_id, member_id=user_id).order_by(Issue.issue_date.desc()).first()
        
    if not issue:
        # Create a digital reading checkout record if none exists
        book = Book.query.get(book_id)
        if book:
            org_id = get_current_org_id() or book.org_id
            issue = Issue(
                book_id=book.id,
                member_id=user_id,
                issue_date=datetime.utcnow(),
                due_date=datetime.utcnow() + timedelta(days=14),
                status='returned', # digital completed or reading
                org_id=org_id,
        ...
        issue.reading_progress = progress_percentage
        issue.current_page = current_page
        db.session.commit()
        
    user = User.query.get(user_id)
    if user:
        if progress_percentage > user.reading_score:
            user.reading_score = min(100, progress_percentage)
            db.session.commit()
            
    # Emit socket io sync event so dashboard and timeline updates
    from flask import current_app
    if hasattr(current_app, 'socketio') and current_app.socketio:
        try:
            current_app.socketio.emit('sync', {'event': 'progress_updated', 'user_id': user_id}, broadcast=True)
            if progress_percentage == 100:
                current_app.socketio.emit('notification', {
                    'title': 'Book Finished!',
                    'message': f"Congratulations on completing '{issue.book.title if issue.book else ''}'!",
                    'timestamp': datetime.utcnow().strftime('%H:%M:%S'),
                    'is_read': False
                }, broadcast=True)
        except Exception as e:
    ...
    return jsonify({
        "msg": "Reading progress saved successfully",
        "reading_progress": progress_percentage,
        "current_page": current_page
    }), 200
