...
class Issue(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'book_title': self.book.title if (self.book and self.book.title) else 'Unknown Title',
            'book_author': self.book.author if (self.book and self.book.author) else 'Unknown Author',
            'book_cover_url': self.book.cover_url if self.book else None,
            'book_pdf_url': self.book.pdf_url if self.book else None,
            'book_category': self.book.category if (self.book and self.book.category) else 'General',
            'member_id': self.member_id,
            'member_name': self.member.username if (self.member and self.member.username) else 'Unknown Member',
            'membership_id': self.member.membership_id if (self.member and self.member.membership_id) else 'N/A',
            'issue_date': self.issue_date.strftime('%Y-%m-%d %H:%M:%S') if self.issue_date else 'N/A',
            'due_date': self.due_date.strftime('%Y-%m-%d %H:%M:%S') if self.due_date else 'N/A',
            'return_date': self.return_date.strftime('%Y-%m-%d %H:%M:%S') if self.return_date else 'N/A',
            'status': self.status or 'issued',
            'fine_amount': self.fine_amount or 0.0,
            'org_id': self.org_id,
