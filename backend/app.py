...
<<<<<<< HEAD
            ...
            try:
                ...
                db.session.rollback()
                try:
                    db.session.execute(db.text('ALTER TABLE issues ADD COLUMN reading_progress INTEGER DEFAULT 0'))
                    db.session.execute(db.text('ALTER TABLE issues ADD COLUMN current_page INTEGER DEFAULT 1'))
                    db.session.commit()
                    print("Self-healing DB migration: added reading progress columns to issues table.")
                except Exception as ex:
                    db.session.rollback()
                    print(f"Failed to migrate issues reading progress: {ex}")

            # Smart Student ID card fields (users table) (NEW)
            try:
                db.session.execute(db.text('SELECT full_name FROM users LIMIT 1'))
            except Exception:
                db.session.rollback()
                try:
                    db.session.execute(db.text('ALTER TABLE users ADD COLUMN full_name VARCHAR(100)'))
                    db.session.execute(db.text('ALTER TABLE users ADD COLUMN roll_number VARCHAR(50)'))
                    db.session.execute(db.text('ALTER TABLE users ADD COLUMN academic_year VARCHAR(50)'))
                    db.session.execute(db.text('ALTER TABLE users ADD COLUMN expiry_date VARCHAR(50)'))
                    db.session.execute(db.text('ALTER TABLE users ADD COLUMN emergency_contact VARCHAR(100)'))
                    db.session.commit()
                    print("Self-healing DB migration: added Smart ID card columns to users table.")
                except Exception as ex:
                    db.session.rollback()
                    print(f"Failed to migrate users Smart ID fields: {ex}")
