...
export default function Dashboard({ user, onTabChange }: DashboardProps) {
  ...
  const categoryChartOptions = {
    ...
    labels: categoryData.labels.length > 0 ? categoryData.labels : ['Computer Science', 'Physics', 'Literature'],
    legend: { labels: { colors: '#94a3b8' }, position: 'bottom' as const },
    stroke: { show: false },
    theme: { mode: 'dark' as const }
  };
  const categoryChartSeries = categoryData.data.length > 0 ? categoryData.data : [8, 5, 12];
  ...
  // 4. Double Series Line - Issues vs Returns
  const issuesReturnsChartOptions = {
    chart: { id: 'issues-returns-trend', toolbar: { show: false }, background: 'transparent' },
    colors: ['#3b82f6', '#10b981'],
    stroke: { curve: 'smooth' as const, width: 3 },
    xaxis: {
      categories: issueTrend.labels.length > 0 ? issueTrend.labels : ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'],
      labels: { style: { colors: '#64748b' } }
    },
    yaxis: { labels: { style: { colors: '#64748b' } } },
    grid: { borderColor: 'rgba(255,255,255,0.03)' },
  ...
  const issuesReturnsChartSeries = [
    { name: 'Monthly Book Issues', data: issueTrend.data.length > 0 ? issueTrend.data : [12, 19, 14, 25, 22, 30] },
    { name: 'Monthly Returns', data: (issueTrend.data.length > 0 ? issueTrend.data : [12, 19, 14, 25, 22, 30]).map((v: number) => Math.max(0, Math.floor(v * 0.85))) }
  ];

  // 5. Area - Fine Collection Trend
  const fineCollectionChartOptions = {
    chart: { id: 'fine-collection-trend', toolbar: { show: false }, background: 'transparent' },
    colors: ['#fbbf24'],
    stroke: { curve: 'smooth' as const, width: 3 },
    fill: { type: 'gradient', gradient: { opacityFrom: 0.35, opacityTo: 0.05 } },
    xaxis: {
      categories: fineTrend.labels.length > 0 ? fineTrend.labels : ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'],
      labels: { style: { colors: '#64748b' } }
    },
    yaxis: { labels: { style: { colors: '#64748b' } } },
    grid: { borderColor: 'rgba(255,255,255,0.03)' },
  ...
  const fineCollectionChartSeries = [
    { name: 'Fine Collection Trend (₹)', data: fineTrend.data.length > 0 ? fineTrend.data : [50, 120, 90, 200, 150, 240] }
  ];

  // 6. Bar - Member Registrations
  const memberRegChartOptions = {
    chart: { id: 'member-reg-trend', toolbar: { show: false }, background: 'transparent' },
    colors: ['#a855f7'],
    stroke: { curve: 'smooth' as const, width: 3 },
    xaxis: {
      categories: issueTrend.labels.length > 0 ? issueTrend.labels : ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'],
  ...
  const memberRegChartSeries = [
    { name: 'Member Registrations', data: (issueTrend.data.length > 0 ? issueTrend.data : [12, 19, 14, 25, 22, 30]).map((v: number) => Math.max(1, Math.floor(v * 0.4 + 2))) }
  ];

  // 7. Horizontal Bar - Top Borrowed Books
  const topBooksList = dashboardData.popular_books || [];
  const topBorrowedChartOptions = {
    chart: { id: 'top-borrowed-books', toolbar: { show: false }, background: 'transparent' },
    plotOptions: {
      bar: {
        horizontal: true,
        barHeight: '60%',
    ...
    colors: ['#ec4899'],
    xaxis: {
      categories: topBooksList.length > 0 ? topBooksList.map((b: any) => b.title.substring(0, 15) + '...') : ['Clean Code', 'Introduction to Algorithms', 'Design Patterns'],
      labels: { style: { colors: '#64748b' } }
    },
    yaxis: { labels: { style: { colors: '#64748b' } } },
    grid: { borderColor: 'rgba(255,255,255,0.03)' },
  ...
  const topBorrowedChartSeries = [
    { name: 'Borrow Count', data: topBooksList.length > 0 ? topBooksList.map((b: any) => b.popularity) : [45, 32, 28] }
  ];
      ...
      >
        ...
        <motion.div variants={containerVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Available Books', value: Math.max(0, kpi.total_books - kpi.books_issued), icon: CheckCircle, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/10' },
            { label: 'Issued Books', value: kpi.books_issued, icon: Bookmark, color: 'text-yellow-400', bg: 'bg-yellow-500/10 border-yellow-500/10' },
            { label: 'Returned Today', value: dashboardData.today_returns_count || 0, icon: CheckCircle, color: 'text-teal-400', bg: 'bg-teal-500/10 border-teal-500/10' },
            { label: 'Overdue Books', value: dashboardData.overdue_books?.length || 0, icon: Clock, color: 'text-red-400', bg: 'bg-red-500/10 border-red-500/10' },
            { label: 'Active Members', value: kpi.total_members, icon: Users, color: 'text-purple-400', bg: 'bg-purple-500/10 border-purple-500/10' },
            { label: 'Pending Requests', value: kpi.reservations, icon: Calendar, color: 'text-cyan-400', bg: 'bg-cyan-500/10 border-cyan-500/10' },
            { label: 'Fine Collection', value: `₹${kpi.revenue.toLocaleString()}`, icon: DollarSign, color: 'text-amber-400', bg: 'bg-amber-500/10 border-amber-500/10' }
          ].map((card, idx) => (
            <motion.div 
              key={idx} 
              variants={itemVariants}
              className="glass-panel p-4 flex items-center justify-between border-white/5 hover:border-white/10"
            >
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">{card.label}</span>
                <h3 className="text-xl font-extrabold text-white mt-1.5">
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="glass-panel p-5 border-white/5 h-[320px] flex flex-col">
            <h4 className="text-white text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-blue-400" /> Monthly Issues & Returns
            </h4>
            <div className="flex-grow min-h-0">
              <Chart options={issuesReturnsChartOptions} series={issuesReturnsChartSeries} type="line" height="100%" />
            </div>
          </div>

          <div className="glass-panel p-5 border-white/5 h-[320px] flex flex-col">
            <h4 className="text-white text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-2">
              <Award className="w-4 h-4 text-cyan-400" /> Popular Categories
            </h4>
            <div className="flex-grow min-h-0 flex items-center justify-center">
              <Chart options={categoryChartOptions} series={categoryChartSeries} type="donut" width="100%" height="85%" />
            </div>
          </div>

          <div className="glass-panel p-5 border-white/5 h-[320px] flex flex-col">
            <h4 className="text-white text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-2">
              <DollarSign className="w-4 h-4 text-emerald-400" /> Fine Collection Trend
            </h4>
            <div className="flex-grow min-h-0">
              <Chart options={fineCollectionChartOptions} series={fineCollectionChartSeries} type="area" height="100%" />
            </div>
          </div>

          <div className="glass-panel p-5 border-white/5 h-[320px] flex flex-col">
            <h4 className="text-white text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-2">
              <Users className="w-4 h-4 text-purple-400" /> Member Registrations
            </h4>
            <div className="flex-grow min-h-0">
              <Chart options={memberRegChartOptions} series={memberRegChartSeries} type="bar" height="100%" />
            </div>
          </div>

          <div className="glass-panel p-5 border-white/5 h-[320px] flex flex-col">
            <h4 className="text-white text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-2">
              <BookOpen className="w-4 h-4 text-pink-400" /> Top Borrowed Books
            </h4>
            <div className="flex-grow min-h-0">
              <Chart options={topBorrowedChartOptions} series={topBorrowedChartSeries} type="bar" height="100%" />
            </div>
          </div>

          <div className="glass-panel p-5 border-white/5 h-[320px] flex flex-col">
            <h4 className="text-white text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-2">
              <Activity className="w-4 h-4 text-yellow-400" /> Fine Trend vs Checkout
            </h4>
            <div className="flex-grow min-h-0">
